...
public abstract class AbsDispatcherActivity extends AppCompatActivity {
    ...
    private void handleUriRequestFromOuter(Uri uri) {
        ...
        com.luojilab.component.componentlib.router.ui.VerifyResult verifyResult = com.luojilab.component.componentlib.router.ui.UIRouter.getInstance().verifyUri(uri, null, true);

        if (verifyResult.isSuccess()) {
            try {
                com.luojilab.component.componentlib.router.ui.UIRouter.getInstance().openUri(this, uri, generateBasicBundle());
            } catch (Exception e) {
                e.printStackTrace();
